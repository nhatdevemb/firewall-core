document.getElementById('importUpdateDevice8').addEventListener('click', function() {
    document.getElementById('fileInput8').click(); // Mở hộp thoại chọn file
});

document.getElementById('fileInput8').addEventListener('change', function(event) {
    ...
    if (fileNames.length > 0) {
        document.getElementById('selectedFileName8').style.display = 'block';
        document.getElementById('fileName8').innerHTML = fileNames.join(', ');
    } else {
...
document.getElementById('capnhat8').addEventListener('click', function() {
    ...
    uploadFilesCustom(files);
});

// Chỉnh sửa phần gửi file để đảm bảo tên trường khớp với tên trong multer
function uploadFilesCustom(files) {
    const loader = document.getElementById('loader8');
